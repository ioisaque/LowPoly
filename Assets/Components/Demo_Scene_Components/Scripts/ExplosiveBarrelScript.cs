...
public class ExplosiveBarrelScript : MonoBehaviour {
	...
	private IEnumerator Explode () {
		...
		foreach (Collider hit in colliders) {
            ...
            //If the explosion hit the tag "Player"
            if (hit.transform.tag == "Player") 
			{
				//Toggle the isHit bool on the target object
				hit.transform.gameObject.GetComponentInParent<PlayerHealth>().Hit = Random.Range(90, 125);
			}

            //If the explosion hit the tag "EnemyBody"
            if (hit.transform.tag == "EnemyBody") 
			{
				//Toggle the isHit bool on the target object
				hit.transform.gameObject.GetComponentInParent<ZombieScript>().Hit = Random.Range(98, 125);
			}
