...
public class GrenadeScript : MonoBehaviour {
	...
	private IEnumerator ExplosionTimer () 
	{
		...
		foreach (Collider hit in colliders) {
			...
			{
                //Toggle "explode" on explosive barrel object
                hit.transform.gameObject.GetComponentInParent<PlayerHealth>().Hit = Random.Range(60, 110);
			}

            //If the explosion hits "EnemyBody" tag
            if (hit.GetComponent<Collider>().tag == "EnemyBody") 
			{
                //Toggle "explode" on explosive barrel object
                hit.transform.gameObject.GetComponentInParent<ZombieScript>().Hit = Random.Range(80, 110);
			}
		}
