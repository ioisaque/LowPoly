...
public class HandgunScriptLPFP : MonoBehaviour {
	...
	private void LateUpdate () {
		//Weapon sway
		if (weaponSway == true) {
			float movementX = -Input.GetAxis ("joystickX") * swayAmount;
			float movementY = -Input.GetAxis ("joystickY") * swayAmount;
	...
	private void Update () {
		...
		if(Input.GetAxis("joystickAim") != 0 && !isReloading && !isRunning && !isInspecting) 
		{
		...
		if (Input.GetKeyDown (KeyCode.JoystickButton3) && !isInspecting) 
		{
			anim.Play ("Knife Attack 1", 0, 0f);
		}
		//Play knife attack 2 animation when F key is pressed
		if (Input.GetAxisRaw("joystickFire") != 0 && !isInspecting) 
		{
			anim.Play ("Knife Attack 2", 0, 0f);
		}
			
		//Throw grenade when pressing G key
		if (Input.GetKeyDown (KeyCode.JoystickButton1) && !isInspecting) 
		{
		...
		//Shooting 
		if (Input.GetKeyDown(KeyCode.JoystickButton5) && !outOfAmmo && !isReloading && !isInspecting && !isRunning) 
		{
			anim.Play ("Fire", 0, 0f);
		...
		if (Input.GetKeyDown (KeyCode.JoystickButton4) && !hasBeenHolstered) 
		{
			holstered = true;
		...
		else if (Input.GetKeyDown (KeyCode.JoystickButton4) && hasBeenHolstered) 
		{
			holstered = false;
		...
		//Reload 
		if (Input.GetKeyDown (KeyCode.JoystickButton2) && !isReloading && !isInspecting) 
		{
			//Reload
			Reload ();
		...
		if (Input.GetAxis("joystickX") != 0 && !isRunning || 
			Input.GetAxis("joystickY") != 0 && !isRunning) 
		{
		...
		if ((Input.GetAxis("joystickX") != 0 && Input.GetKey (KeyCode.JoystickButton8))) 
		{
			isRunning = true;
		} else {
			isRunning = false;
